//代码来自：https://github.com/opp100/bmw-scriptable-widgets
//进行了部分删减
...
class Widget extends Base {
    ...
    constructor(arg) {
        super(arg);
        this.name = 'BMW-Linker-Mini';
        this.desc = 'My BMW互联Widget';
        ...
        if (config.runsInApp) {
            ...
            this.registerAction('登录My BMW', this.userLoginCredentials);
        }
    }
    ...
    async userLoginCredentials() {
        ...
        userLoginAlert.addTextField('账号', this.userConfigData['username']);
        userLoginAlert.addSecureTextField('密码', this.userConfigData['password']);
        ...
        return this.getVehicleData(true);
    }
        ...
        return await Promise.all(
            DEPENDENCIES.map(async (js) => {
                try {
                    ...
                    if (!fileExists) {
                        this.notify('Error', '下载依赖文件失败，请重新尝试配置');
                    }
    ...
    async userConfigInput() {
        ...
        userCustomConfigAlert.message = '留空信息会从系统自动获取';
        ...
        let configSet = {
            ...
            vin: '车架号(多辆BMW时填写)',
            force_dark_theme: '使用深色主题（Y or N）'
        };
        ...
        for (const key in configSet) {
            ...
            if (key == 'force_dark_theme') {
                userCustomConfigAlert.addTextField(configSet[key], this.userConfigData[key] ? 'Y' : null);
                continue;
            }
        ...
        for (const key in configSet) {
            ...
            if (key == 'force_dark_theme') {
                this.userConfigData[key] = this.userConfigData[key] && this.userConfigData[key] == 'Y';
            }
        }
    ...
    async render() {
        ...
        await this.renderError('Loading...');
        ...
        //vehicleData
        let data = await this.getVehicleData();
        ...
        if (
            ...
            data = {
                status: {
                    ...
                    currentMileage: {mileage: 8888, units: 'km'}
                }
            };
        }
    ...
    async renderMedium(data, renderLarge = false) {
        ...
        let renderMediumContent = !renderLarge;
        ...
        if (renderMediumContent) {
            ...
            if (data.status && data.status.doorsAndWindows && data.status.doorsAndWindows.length > 0) {
                ...
                windowStatusContainer.setPadding(8, 0, 10, 0);
    ...
    async getVehicleData(forceRefresh = false) {
    ...
    async getVehicleDetails(accesstoken, forceRefresh = false) {
        ...
        return vehicleData ? vehicleData : this.loadVehicleFromCache(vin)
    }
